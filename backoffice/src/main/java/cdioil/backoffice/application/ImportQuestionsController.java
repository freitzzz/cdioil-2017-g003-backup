...
import cdioil.application.utils.QuestionssReader;
...
public class ImportQuestionsController {
    ...
    public int lerFicheiro(String filename) {
        QuestionssReader questionsReader = QuestionsReaderFactory.create(filename);
        if (questionsReader != null) {
            int cql = questionsReader.readQuestoes(globalLibrary, manager);
            try {
                new GlobalLibraryRepositoryImpl().merge(globalLibrary);
                return cql;
            } catch (IllegalArgumentException ex) {
                System.out.println("As Questões não foram persistidas.");
            }
        }
        return 1;
    }
