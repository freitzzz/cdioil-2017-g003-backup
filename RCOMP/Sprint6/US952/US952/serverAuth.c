...
int main(int argc,char *argv[]){
    ...
    req.ai_family = AF_UNSPEC; 				//Defines the family of the connection (IPv4 & IPv6)
    req.ai_socktype = SOCK_STREAM; 			//Defines the type of socket to use (TCP)
    req.ai_flags = AI_PASSIVE; 				// local address
	
	
	//UDP CONNECTION INFO
	struct addrinfo udp_req, *udp_list;
	bzero((char *)&udp_req, sizeof(udp_req));
	udp_req.ai_family = AF_INET;
	udp_req.ai_socktype = SOCK_DGRAM;
	udp_req.ai_flags = AI_PASSIVE;
	...
	int udp_err = getaddrinfo(NULL, UDP_PORT, &udp_req, &udp_list);
	if(udp_err) return 0;

	int udp_sock = socket(udp_list->ai_family, udp_list->ai_socktype, udp_list->ai_protocol);
	if(udp_sock == -1) return 0;

	int udp_binded = bind(udp_sock, (struct sockaddr *)udp_list->ai_addr, udp_list->ai_addrlen);

	if(udp_binded == -1) return 0;

	freeaddrinfo(udp_list);

    int failure=getaddrinfo(NULL, TCP_PORT, &req, &list); //Fills struct with necessary info
    if(failure)return 0;
    	
    int tcp_sock = socket(list->ai_family, list->ai_socktype, list->ai_protocol);	//Creates a socket for listening to new connections
    if(tcp_sock==-1)return 0;
    
    int binded=bind(tcp_sock, (struct sockaddr *)list->ai_addr, list->ai_addrlen);
    ...
    while(1){
		...
		recvfrom(udp_sock, NULL, 0, 0, (struct sockaddr *)&client_address, &address_length);

		if(!getnameinfo((struct sockaddr *)&client_address, address_length, clientIP, BUF_SIZE,
                            clientPort, BUF_SIZE, NI_NUMERICHOST | NI_NUMERICSERV)){
			printf("Received UDP connection from client %s, port %s\n", clientIP, clientPort);
		}

		//send server IP address
		sendto(udp_sock, NULL, 0, 0, (struct sockaddr *)&client_address, address_length);

        int newSock = accept(tcp_sock, (struct sockaddr *)&client_address, &address_length);
        if(!fork()){
            ...
            getnameinfo((struct sockaddr *)&client_address, address_length, clientIP, BUF_SIZE,
                            clientPort, BUF_SIZE, NI_NUMERICHOST | NI_NUMERICSERV);
